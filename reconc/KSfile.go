...
func (cf *KunshanZJFile) Init(chainName string, mc string) gerror.IError {
...
func (cf *KunshanZJFile) SaveToFile() gerror.IError {
	//读数据
	gerr := cf.ReadDate()
	if gerr != nil {
...
func (cf *KunshanZJFile) postToSftp(fileName string, fileData []byte) {
...
func (cf *KunshanZJFile) Finish() {
	return
}

func (cf *KunshanZJFile) ReadDate() gerror.IError {
	dbc := gormdb.GetInstance()
	var rows *sql.Rows
	var err error
...
func (cf *KunshanZJFile) GetInsIdCd() (string, bool) {
	l := len(cf.Ins_id_cd)
	if l == 0 {
		return "", false
	}
...
func (cf *KunshanZJFile) geneFile() string {
	cd, ok := cf.GetInsIdCd()
	if ok {
...
func (cf *KunshanZJFile) InitMCHTCd(mc string) gerror.IError {
