...
type LiuzhouZJFile struct {
	...
	FileStrt             models.LZFileStrt          //文件结构
...
func (cf *LiuzhouZJFile) Init(chainName string, mc string) gerror.IError {
...
func (cf *LiuzhouZJFile) SaveToFile() gerror.IError {
	//读数据
	gerr := cf.ReadDate()
	if gerr != nil {
...
func (cf *LiuzhouZJFile) postToSftp(fileName string, fileData []byte) {
	...
	for rows.Next() {
		...
		if cf.sendto {
			...
			case "0":
				...
				name := "NNGPOS_"+cf.Pay_DATE_E + ".txt"
...
func (cf *LiuzhouZJFile) Finish() {
	return
}

func (cf *LiuzhouZJFile) ReadDate() gerror.IError {
	dbc := gormdb.GetInstance()
	var rows *sql.Rows
	var err error
	...
	if len(cf.Tbl_Clear_Data) > 0 || cf.empty {
		cf.FileStrt.LZFileHeadInfo.Area_CD = cf.Area_cd				//服务商地区代码
	} else {
		return gerror.NewR(0000, err, "MCHT_CD[%s]记录不存在", cf.MCHT_CD)
	}
...
func (cf *LiuzhouZJFile) saveDatatoFStru() gerror.IError {
	//cf.FileStrt.FileBodys = make([]models.Body,0)
	cf.FileStrt.FileBodys = []models.LZBody{}
	dbc := gormdb.GetInstance()
	...
	for _, tc := range cf.Tbl_Clear_Data {
		...
		b.JC_APL_CD = tran.CUST_ORDER_ID								//交存申请编码
	...
	cf.FileStrt.LZFileHeadInfo.Stlm_date = cf.Pay_DATE_E
	cf.FileStrt.LZFileHeadInfo.TrnReconT = strconv.FormatFloat(trnrecont_T, 'f', 2, 64)		//交易总结算额
	logr.Info("成功总笔数:", record)

	cf.FileStrt.LZFileHeadInfo.TrnSucCount = strconv.Itoa(record)			//成功总笔数
	return nil
}

func (cf *LiuzhouZJFile) GetInsIdCd() (string, bool) {
	l := len(cf.Ins_id_cd)
	if l == 0 {
		return "", false
	}
...
func (cf *LiuzhouZJFile) geneFile() string {
	cd, ok := cf.GetInsIdCd()
	if ok {
		cf.FileName = "NNGPOS_" + cd
	} else {
		return ""
	}
...
func (cf *LiuzhouZJFile) InitMCHTCd(mc string) gerror.IError {
