...
type QuzhouZJFile struct {
	...
	FileStrt             models.QZFileStrt          //文件结构
...
func (cf *QuzhouZJFile) Init(chainName string, mc string) gerror.IError {
...
func (cf *QuzhouZJFile) SaveToFile() gerror.IError {
	//读数据
	gerr := cf.ReadDate()
	if gerr != nil {
...
func (cf *QuzhouZJFile) postToSftp(fileName string, fileData []byte) {
...
func (cf *QuzhouZJFile) Finish() {
	return
}

func (cf *QuzhouZJFile) ReadDate() gerror.IError {
	dbc := gormdb.GetInstance()
	var rows *sql.Rows
	var err error
	...
	if len(cf.Tbl_Clear_Data) > 0 || cf.empty {
		cf.FileStrt.QZFileHeadInfo.Area_CD = cf.Area_cd				//服务商地区代码
	} else {
		return gerror.NewR(0000, err, "MCHT_CD[%s]记录不存在", cf.MCHT_CD)
	}
...
func (cf *QuzhouZJFile) saveDatatoFStru() gerror.IError {
	//cf.FileStrt.FileBodys = make([]models.Body,0)
	cf.FileStrt.FileBodys = []models.QZBody{}
	dbc := gormdb.GetInstance()
	...
	for _, tc := range cf.Tbl_Clear_Data {
		...
		b.GF_BIZ_CD = tran.Ext_fld7												//监管子账户
	...
	cf.FileStrt.QZFileHeadInfo.Stlm_date = cf.Pay_DATE_E
	cf.FileStrt.QZFileHeadInfo.TrnReconT = strconv.FormatFloat(trnrecont_T, 'f', 2, 64)		//交易总结算额
	logr.Info("成功总笔数:", record)

	cf.FileStrt.QZFileHeadInfo.TrnSucCount = strconv.Itoa(record)			//成功总笔数
	return nil
}

func (cf *QuzhouZJFile) GetInsIdCd() (string, bool) {
	l := len(cf.Ins_id_cd)
	if l == 0 {
		return "", false
	}
...
func (cf *QuzhouZJFile) geneFile() string {
	cd, ok := cf.GetInsIdCd()
	if ok {
...
func (cf *QuzhouZJFile) InitMCHTCd(mc string) gerror.IError {
