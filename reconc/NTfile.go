...
type NantongZJFile struct {
	...
	FileStrt             models.NTFileStrt          //文件结构
...
func (cf *NantongZJFile) Init(chainName string, mc string) gerror.IError {
...
func (cf *NantongZJFile) SaveToFile() gerror.IError {
	//读数据
	gerr := cf.ReadDate()
	if gerr != nil {
...
func (cf *NantongZJFile) postToSftp(fileName string, fileData []byte) {
...
func (cf *NantongZJFile) Finish() {
	return
}

func (cf *NantongZJFile) ReadDate() gerror.IError {
	dbc := gormdb.GetInstance()
	var rows *sql.Rows
	var err error
	...
	if len(cf.Tbl_Clear_Data) > 0 || cf.empty {
		cf.FileStrt.NTFileHeadInfo.Area_CD = cf.Area_cd				//服务商地区代码
	} else {
		return gerror.NewR(0000, err, "MCHT_CD[%s]记录不存在", cf.MCHT_CD)
	}
...
func (cf *NantongZJFile) saveDatatoFStru() gerror.IError {
	//cf.FileStrt.FileBodys = make([]models.Body,0)
	cf.FileStrt.FileBodys = []models.NTBody{}
	dbc := gormdb.GetInstance()
	...
	cf.FileStrt.NTFileHeadInfo.Stlm_date = cf.Pay_DATE_E
	cf.FileStrt.NTFileHeadInfo.TrnReconT = strconv.FormatFloat(trnrecont_T, 'f', 2, 64)		//交易总结算额
	logr.Info("成功总笔数:", record)

	cf.FileStrt.NTFileHeadInfo.TrnSucCount = strconv.Itoa(record)			//成功总笔数
	return nil
}

func (cf *NantongZJFile) GetInsIdCd() (string, bool) {
	l := len(cf.Ins_id_cd)
	if l == 0 {
		return "", false
	}
...
func (cf *NantongZJFile) geneFile() string {
	cd, ok := cf.GetInsIdCd()
	if ok {
...
func (cf *NantongZJFile) InitMCHTCd(mc string) gerror.IError {
